...
function renderHome(m){
  ...
  if(m&&m.weather){
    if($('#weatherIcon')) $('#weatherIcon').textContent=(m.weather.iconEmoji||'☁️');
  ...
  if($('#energyValue')) $('#energyValue').textContent=(ev!=null?String(ev)+' kWh':'— kWh');
  if($('#lblOverride')) $('#lblOverride').textContent=(m?.override?'On':'Off');
  if($('#lblVacanza'))  $('#lblVacanza').textContent=(m?.vacanza?'On':'Off');
  if($('#btnOverride')) $('#btnOverride').classList.toggle('on',!!m?.override);
  if($('#btnVacanza'))  $('#btnVacanza').classList.toggle('on',!!m?.vacanza);
  ...
  if($('#peopleSummary')) $('#peopleSummary').textContent=`${on} online / ${ppl.length} totali`;
}
...
function renderEnergyPage(m){
  ...
  if($('#e2Today'))   $('#e2Today').textContent=(m.energy?.kwh!=null?`${(m.energy.kwh*0.6).toFixed(1)} kWh`:'--');
  if($('#e2Week'))    $('#e2Week').textContent=(m.energy?.kwh!=null?`${(m.energy.kwh*4).toFixed(1)} kWh`:'--');
