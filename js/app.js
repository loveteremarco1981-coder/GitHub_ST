/* =======================================================================
 * Automazione UI — OneConnect v2.3 (ottimizzato)
 * - Usa api.js per gli endpoint admin (get/set/diagnostica/keepalive)
 * - Unico JSONP "anonimo" per MODEL (endpoint pubblico)
 * - Navigazione, bind, rendering pagine
 * ======================================================================= */
...
/* -------------------- JSONP (solo per MODEL) ----------------- */
/** Usa il valore globale EXEC_URL definito in api.js */
function jsonpModel(path=''){
  return new Promise((resolve,reject)=>{
    try{
      ...
      const url  = `${EXEC_URL}${path}${path.includes('?')?'&':'?'}callback=${cb}&t=${Date.now()}`;
      const s    = document.createElement('script');
      s.src = url; s.onerror = (e)=>{ try{ delete window[cb]; }catch(_){ } reject(e); };
      document.body.appendChild(s);
      setTimeout(()=>{ try{s.remove();}catch(_){ } }, 5000);
    }catch(e){ reject(e); }
  });
}
...
async function loadPeople(){
  try{
    const res = await jsonpModel('?people=1'); // endpoint pubblico
...
function wireSettings(){
  ...
  $('#btnSaveStrict')?.addEventListener('click', async ()=>{
    const ok = await saveNumber('set_strict', $('#inpStrict').value); toast(ok?'Salvato':'Errore'); if(ok) await loadModel();
  });
  $('#btnSaveHold')?.addEventListener('click', async ()=>{
    const ok = await saveNumber('set_hold', $('#inpHold').value); toast(ok?'Salvato':'Errore'); if(ok) await loadModel();
  });
  ...
  $('#btnToggleOverride')?.addEventListener('click', async ()=>{
    try{
      ...
      const f2 = await apiFetch('get_flags'); const on = !!(f2?.ok && f2.override);
      $('#lblOverrideState') && ($('#lblOverrideState').textContent = on?'ON':'OFF');
      $('#lblOverride')      && ($('#lblOverride').textContent      = on?'On':'Off');
      $('#btnOverride')      && $('#btnOverride').classList.toggle('on', on);
      $('#btnToggleOverride')&& $('#btnToggleOverride').classList.toggle('on', on);
      toast('Override: '+(on?'ON':'OFF')); await loadModel();
  ...
  $('#btnToggleVacanza')?.addEventListener('click', async ()=>{
    try{
      ...
      const f2 = await apiFetch('get_flags'); const on = !!(f2?.ok && f2.vacanza);
      $('#lblVacanzaState') && ($('#lblVacanzaState').textContent = on?'ON':'OFF');
      $('#lblVacanza')      && ($('#lblVacanza').textContent      = on?'On':'
